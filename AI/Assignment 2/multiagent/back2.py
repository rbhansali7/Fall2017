...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def minimax(self, gameState, isPac, ghostNum, depth):
        ...
        if isPac:
            ...
            if len(legalMoves) > 0:
                scores = [self.minimax(gameState.generateSuccessor(0, action), False, 1, depth + 1) for action in
                          legalMoves]
                bestScore = max(scores)
                bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore]
                bestAction = legalMoves[bestIndices[0]]
                if depth == 0:
                    # print bestAction
                    return bestAction
                return bestScore
        ...
        if not isPac:
            ...
            if len(legalMoves) > 0:
                if ghostNum < gameState.getNumAgents() - 1:
                    scores = [
                        self.minimax(gameState.generateSuccessor(ghostNum, action), False, ghostNum + 1, depth + 1) for
                        action in legalMoves]
                    bestScore = min(scores)
                    return bestScore
                else:
                    scores = [self.minimax(gameState.generateSuccessor(ghostNum, action), True, ghostNum + 1, depth + 1)
    ...
    def minimaxAB(self, gameState, isPac, ghostNum, depth, alpha, beta):
        ...
        if isPac:
            ...
            if len(legalMoves) > 0:
                for i in range(len(legalMoves)):
                    score = self.minimaxAB(gameState.generateSuccessor(0, legalMoves[i]), False, 1, depth + 1, alpha,
                                           beta)
                    if score > bestScore:
                        bestScore = score
        ...
        if not isPac:
            ...
            if len(legalMoves) > 0:
                if ghostNum < gameState.getNumAgents() - 1:
                    for i in range(len(legalMoves)):
                        score = self.minimaxAB(gameState.generateSuccessor(ghostNum, legalMoves[i]), False,
                                               ghostNum + 1, depth + 1, alpha, beta)
                        if score < bestScore:
                            bestScore = score
                    ...
                    for i in range(len(legalMoves)):
                        score = self.minimaxAB(gameState.generateSuccessor(ghostNum, legalMoves[i]), True, 0, depth + 1,
                                               alpha, beta)
                        if score < bestScore:
                            bestScore = score
                        beta = min(beta, bestScore)
    ...
    def expectimax(self, gameState, isPac, ghostNum, depth):
        ...
        if not isPac:
            ...
            if len(legalMoves) > 0:
                if ghostNum < gameState.getNumAgents() - 1:
                    for i in range(len(legalMoves)):
                        score = self.expectimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), False,
                                                ghostNum + 1, depth + 1)
                        if score < bestScore:
                            bestScore = score
                    return bestScore
                    ...
                    for i in range(len(legalMoves)):
                        score = self.expectimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), True, 0,
                                                depth + 1)
                        if score < bestScore:
                            bestScore = score
                    return bestScore
