...
class MinimaxAgent(MultiAgentSearchAgent):
        ...
        if isPac:
            ...
            if len(legalMoves)>0:
                for i in range(len(legalMoves)):
                    score = self.minimax(gameState.generateSuccessor(0, legalMoves[i]), False, 1, depth+1)
                    if score> bestScore:
                        bestScore = score
        ...
        if not isPac:
            ...
            if len(legalMoves)>0:
                if ghostNum<gameState.getNumAgents()-1:
                    for i in range(len(legalMoves)):
                        score = self.minimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), False, ghostNum+1, depth + 1)
                        if score < bestScore:
                            bestScore = score
                    return bestScore
                else:
                    for i in range(len(legalMoves)):
                        score = self.minimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), True, 0, depth + 1)
                        if score < bestScore:
                            bestScore = score
                    return bestScore
            else:
    ...
    def getAction(self, gameState):
        ...
        return self.minimax(gameState, True, 0, 0)
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def minimaxAB(self, gameState, isPac, ghostNum, depth, alpha, beta):
        ...
        if isPac:
            ...
            if len(legalMoves)>0:
                for i in range(len(legalMoves)):
                    score = self.minimaxAB(gameState.generateSuccessor(0, legalMoves[i]), False, 1, depth+1, alpha, beta)
                    if score> bestScore:
                        bestScore = score
                        bestAction = legalMoves[i]
        ...
        if not isPac:
            ...
            if len(legalMoves)>0:
                if ghostNum<gameState.getNumAgents()-1:
                    for i in range(len(legalMoves)):
                        score = self.minimaxAB(gameState.generateSuccessor(ghostNum, legalMoves[i]), False, ghostNum+1, depth + 1, alpha, beta)
                        if score < bestScore:
                            bestScore = score
                    ...
                    for i in range(len(legalMoves)):
                        score = self.minimaxAB(gameState.generateSuccessor(ghostNum, legalMoves[i]), True, 0, depth + 1, alpha, beta)
                        if score < bestScore:
                            bestScore = score
                        beta = min(beta, bestScore)
                        if (beta<alpha):
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.minimaxAB(gameState, True, 0, 0, -10000000, 10000000)
        util.raiseNotDefined()
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    """
    def expectimax(self, gameState, isPac, ghostNum, depth):
        ...
        if isPac:
            ...
            if len(legalMoves) > 0:
                for i in range(len(legalMoves)):
                    score = self.expectimax(gameState.generateSuccessor(0, legalMoves[i]), False, 1, depth + 1)
                    if score > bestScore:
                        bestScore = score
                        bestAction = legalMoves[i]
                if depth == 0:
        ...
        if not isPac:
            ...
            if len(legalMoves) > 0:
                ...
                if ghostNum < gameState.getNumAgents() - 1:
                    for i in range(len(legalMoves)):
                        score += self.expectimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), False, ghostNum + 1, depth + 1)
                    return float(score)/len(legalMoves)
                else:
                    for i in range(len(legalMoves)):
                        score += self.expectimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), True, 0, depth + 1)
                    return float(score) / len(legalMoves)
            else:
                return self.evaluationFunction(gameState)
        ...
        if isPac==2:
            ...
            if len(legalMoves)>0:
                ...
                if ghostNum < gameState.getNumAgents() - 1:
                    for i in range(len(legalMoves)):
                        score += self.expectimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), 1, ghostNum+1, depth + 1)
                    return float(score)/len(legalMoves)
                else:
                    for i in range(len(legalMoves)):
                        score += self.expectimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), 0, 0, depth + 1)
                    return float(score)/len(legalMoves)
            else:
                return self.evaluationFunction(gameState)
        ...
        if isPac==0:
            ...
            if len(legalMoves)>0:
                for i in range(len(legalMoves)):
                    score = self.expectimax(gameState.generateSuccessor(0, legalMoves[i]), 2, 1, depth+1)
                    if score> bestScore:
                        bestScore = score
                        bestAction = legalMoves[i]
                if depth==0:
        ...
        if isPac==1:
            ...
            if len(legalMoves)>0:
                if ghostNum<gameState.getNumAgents()-1:
                    for i in range(len(legalMoves)):
                        score = self.expectimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), 2, ghostNum+1, depth + 1)
                        if score < bestScore:
                            bestScore = score
                    return bestScore
                else:
                    for i in range(len(legalMoves)):
                        score = self.expectimax(gameState.generateSuccessor(ghostNum, legalMoves[i]), 2, 0, depth + 1)
                        if score < bestScore:
                            bestScore = score
                    return bestScore
            else:
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.expectimax(gameState, True, 0, 0)
        util.raiseNotDefined()
